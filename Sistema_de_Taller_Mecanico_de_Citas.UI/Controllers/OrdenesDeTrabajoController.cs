    ...
    public class OrdenesDeTrabajoController : Controller
    {
        ...
        public async Task<IActionResult> Index(string filtroCliente)
        {
            ...
            try
            {
                ...
                listaDeOrdenes = JsonConvert.DeserializeObject<List<OrdenDeTrabajo>>(content) ?? new List<OrdenDeTrabajo>();
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            try
            {
                ...
                response.EnsureSuccessStatusCode();
                var content = await response.Content.ReadAsStringAsync();
                var lista = JsonConvert.DeserializeObject<List<OrdenDeTrabajo>>(content) ?? new List<OrdenDeTrabajo>();
        ...
        public async Task<IActionResult> IniciarOrden(int? id)
        {
            ...
            try
            {
                ...
                var listaOrdenes = JsonConvert.DeserializeObject<List<OrdenDeTrabajo>>(contentOrden) ?? new List<OrdenDeTrabajo>();
        ...
        private async Task CargarDatosViewBag()
        {
            try
            {
                ...
                var listaClientes = JsonConvert.DeserializeObject<List<Cliente>>(contentClientes) ?? new List<Cliente>();
        ...
        private async Task CargarMecanicosViewBag()
        {
            try
            {
                ...
                var listaMecanicos = JsonConvert.DeserializeObject<List<Mecanico>>(contentMecanicos) ?? new List<Mecanico>();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Finalizar(int id, string metodoPago)
        {
            if (string.IsNullOrEmpty(metodoPago))
            {
                ModelState.AddModelError("metodoPago", "Debe seleccionar un método de pago.");
                return View(new OrdenDeTrabajo { Id = id });
            }

            try
            {
                var model = new OrdenDeTrabajo { Id = id, MetodoPago = metodoPago };
                var jsonContent = JsonConvert.SerializeObject(model);
                var content = new StringContent(jsonContent, Encoding.UTF8, "application/json");
                var response = await _client.PostAsync("api/OrdenesDeTrabajo/FinalizarOrden", content);
                response.EnsureSuccessStatusCode();
